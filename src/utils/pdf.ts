...
import { BulletinPaie, Paiement } from '../generated/prisma';

export class PDFService {
  generatePayslipPDF(payslip: BulletinPaie & { employe: any; cyclePaie: any; paiements: Paiement[] }) {
    return new Promise<Buffer>((resolve, reject) => {
      try {
        ...
        if (payslip.paiements.length > 0) {
          ...
          doc.fontSize(12);
          payslip.paiements.forEach((payment: Paiement) => {
  ...
  generatePaymentReceiptPDF(payment: Paiement & { bulletinPaie: BulletinPaie & { employe: any; cyclePaie: any } }) {
    return new Promise<Buffer>((resolve, reject) => {
      try {
        const doc = new PDFDocument();
        const buffers: Buffer[] = [];
  ...
  generatePaymentListPDF(payments: (Paiement & { bulletinPaie: BulletinPaie & { employe: any } })[], companyName: string) {
    return new Promise<Buffer>((resolve, reject) => {
      try {
        const doc = new PDFDocument();
        const buffers: Buffer[] = [];
        ...
        payments.forEach((payment: Paiement & { bulletinPaie: BulletinPaie & { employe: any } }) => {
